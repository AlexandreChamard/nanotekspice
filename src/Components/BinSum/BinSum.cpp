...
** BinSum.cpp for cpp_nanotekspice
** File description:
** description
*/
...
unsigned int nts::CbinSum::id = 0;

nts::CbinSum::CbinSum(std::string const &value):
_id{ value }
{
	if (_id.size() == 0) {
...
nts::Tristate nts::CbinSum::compute(std::size_t pin)
{
	if (pin > _nbPins) {
...
void nts::CbinSum::setLink(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
	if (pin > _nbPins) {
		throw PinNExistError{ _id, pin };
	}
